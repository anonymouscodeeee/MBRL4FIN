...
class GaussianMLP(Ensemble):
    ...
    def __init__(
        ...
        # only for the propagation_method "mask_expectation"
        self.mask = None
    ...
    def _default_forward(
        ...
        if self.deterministic:
            ...
            # ...表示保留全部，此处a[... , b]
            mean = mean_and_logvar[
                ..., : self.out_size
            ]  # mean.shape [5, 32, 1];mean_and_logvar.shape [5, 32, 2]
    ...
    def _forward_ensemble(
        ...
        if self.propagation_method == "mask":
            mean, logvar = self._default_forward(x, only_elite=True)
            uncertainty, k = self.calc_uncertainty(mean, logvar.exp())
            mask_ratio = self.mask_ratio if self.mask_ratio is not None else self.min_mask_ratio
            mask_num = max(int(mask_ratio * uncertainty.shape[0]), 1)
            self.uncertainty = uncertainty.mean(dim=-1)
            id_k_min = self.uncertainty.topk(mask_num, largest=False).indices  
            self.mask = torch.zeros((mean.shape[1],), device=mean.device).bool()
            self.mask[id_k_min] = True          
            return mean.gather(0, k).squeeze(), logvar.gather(0, k).squeeze()
    ...
    def update_mask_ratio(self, h_max, h):
        self.mask_ratio = self.min_mask_ratio if h_max==1 else (h_max-h)/(2*(h_max+1))

    def calc_uncertainty(self, mean, var):
        # mean, var: [ensemble_size, batch]
        # One-vs-Rest (OvR) uncertainty
        # https://arxiv.org/abs/2010.04893
        ensemble_size, batchsize, fea_size = mean.shape[0], mean.shape[1], mean.shape[2]
        k = torch.randint(
            0, ensemble_size, (1, batchsize, fea_size), device=mean.device
        )
        mean_k = mean.gather(dim=0, index=k)
        var_k = var.gather(dim=0, index=k)
        idx_rest = torch.ones_like(mean).scatter(0, k, 0).bool()

        mean_rest = (
            mean[idx_rest].view(-1, batchsize, fea_size).mean(dim=0, keepdim=True)
        )
        var_rest = (
            var[idx_rest].view(-1, batchsize, fea_size)
            + torch.pow(mean[idx_rest].view(-1, batchsize, fea_size), 2)
        ).mean(dim=0, keepdim=True) - torch.pow(mean_rest, 2)

        # 两个正态分布之间的KL散度
        # kl(N1 | N2)
        n1 = Normal(mean_rest, var_rest + 1e-8)
        n2 = Normal(mean_k, var_k + 1e-8)

        return kl_divergence(n1, n2).squeeze(), k
