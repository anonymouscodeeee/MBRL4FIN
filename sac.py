...
from mbrl.third_party.pytorch_sac.agent import critic
...
class SACAgent(Agent):
    ...
    @torch.no_grad()
    def calc_loss(self, obs, action, reward, next_obs, not_done):
        ######## critic loss
        ...
        target_Q1, target_Q2 = self.critic_target(next_obs, next_action)#Q(s1,a1)下一时刻的期望值
        #如何理解target_V和log_prob的关系
        target_V = torch.min(target_Q1, target_Q2) - self.alpha.detach() * log_prob
        target_Q = reward + (not_done * self.discount * target_V)
        target_Q = target_Q.detach()

        # get current Q estimates
        current_Q1, current_Q2 = self.critic(obs, action)#Q(s0,a0)时的期望值
    ...
    def save_stats(self):
        stats = {"critic_loss":np.array(self.critic_loss), "actor_loss":np.array(self.actor_loss)}
        np.save(self.stat_path,stats, True)
...
#记录Q_target作为Q_value
    def update_critic(self, obs, action, reward, next_obs, not_done, logger, step):
        ...
        target_Q1, target_Q2 = self.critic_target(next_obs, next_action)#Q(s1,a1)下一时刻的期望值
        #如何理解target_V和log_prob的关系
        target_V = torch.min(target_Q1, target_Q2) - self.alpha.detach() * log_prob#可以按照优势函数进行理解
        target_Q = reward + (not_done * self.discount * target_V)
        target_Q = target_Q.detach()
